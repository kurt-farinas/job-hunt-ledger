...
export function Filters({ filters, onChange, total }: FiltersProps) {
  ...
  const clear = () => onChange({ sort_by: 'date_found', sort_order: 'desc', page: 1, page_size: 25 })
  return <section className="filter-panel" aria-labelledby="filter-title">
    ...
    <div className="filter-grid">
      ...
      <label className="field"><span>Status</span><select value={filters.status || ''} onChange={event => set('status', (event.target.value || undefined) as JobStatus | undefined)}><option value="">All statuses</option>{statuses.map(status => <option key={status}>{status}</option>)}</select></label>
