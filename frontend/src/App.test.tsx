...
test('renders the job ledger, match evidence, salary, and safe external link', async () => {
  ...
  expect(api.jobs).toHaveBeenCalledWith(expect.objectContaining({ status: 'New', stale: false }))
...
test('manual status selection and explicit note save use restricted job patches', async () => {
  ...
  await waitFor(() => expect(screen.queryByRole('button', { name: 'Junior React Developer' })).not.toBeInTheDocument())
  expect(screen.getByText("You're caught up")).toBeInTheDocument()
  await user.selectOptions(screen.getByLabelText('Status'), 'Applied')
  const notes = await screen.findByLabelText('Notes for Junior React Developer')
...
test('Find jobs now disables while running then shows counts and source failures separately', async () => {
  ...
  expect(controls.getByLabelText('Status')).toHaveValue('New')
  expect(controls.getByLabelText('Freshness')).toHaveValue('false')
  ...
  expect(within(result).getByText(/12 new jobs added, 36 existing jobs updated, 2 jobs marked stale.*showing fresh jobs/i)).toBeInTheDocument()
